...
const Accordion = ({ faq, toggled, onShow, index }) => {
  return (
    <div className="relative">
      <div className="flex flex-col items-start p-7 bg-gradient-to-br from-gray-200 to-muted-200">
        <div
          id={faq.title}
          onClick={onShow}
        ...
        >
          <span className="lg:mr-4 font-sans3 text-lg lg:text-5xl font-thin w-6 lg:w-24 text-accent-500 tracking-tight overflow-hidden text-left">
            {index + 1}
          </span>
          ...
          {toggled ? (
            <FaMinus className="mr-3 text-primary-800 pointer-events-none" />
          ) : (
            <FaPlus className="mr-3 text-primary-800 pointer-events-none" />
          )}
        </div>
      </div>
      <div
        className={`overflow-hidden px-9 text-muted-800 font-sans2 font-normal w-full ${
          toggled
